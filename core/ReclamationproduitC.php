...
function afficherReclamationproduit ($reclamationproduit){
		...
		echo "etat: ".$reclamationproduit->getEtat()."<br>"; 
	}
	...
	function ajouterReclamationproduit($reclamationproduit){
		$sql="insert into reclamationproduit (id,idClt,typeP,categorieP,numeroP,descriptionP,numerofacture,etat) values (:id,:idClt,:typeP,:categorieP,:numeroP,:descriptionP,:numerofacture,:etat)";
		...
		$etat=$reclamationproduit->getEtat();
		$req->bindValue(':id',$id);
	...
	function modifierReclamationproduit($Reclamationproduit,$id){
		$sql="UPDATE reclamationproduit SET id=:idd,idClt=:idClt,typeP=:typeP,categorieP=:categorieP,numeroP=:numeroP ,descriptionP=:descriptionP ,numerofacture=:numerofacture,etat=:etat WHERE id=:id";
...
try{		
		...
		$numerofacture=$Reclamationproduit->getNumerofacture();
		$etat=$Reclamationproduit->getEtat();
		$datas = array(':idd'=>$idd, ':id'=>$id,':idClt'=>$idClt,':typeP'=>$typeP,':categorieP'=>$categorieP,':numeroP'=>$numeroP,':descriptionP'=>$descriptionP,':numerofacture'=>$numerofacture,':etat'=>$etat);
	...
	function traiterReclamation($id)
 {
	...
	function filtrerReclamationproduit($f) 
 {

       $query = "select * from reclamationproduit where etat LIKE ?";
		 	$db = config::getConnexion();
         $result = $db->prepare($query);
